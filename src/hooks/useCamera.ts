...
export function useCamera() {
  ...
  const capture = useCallback((): Promise<Blob | null> => {
    return new Promise<Blob | null>((resolve) => {
      ...
      // Mobile browsers (especially Android) already provide the video track in the correct upright orientation.
      // Manually rotating it breaks horizontal photos when orientation lock is on.
      canvas.width = vw;
      canvas.height = vh;
